...
N_BINS = 40
BASE_DIR = r'/depot/cms/top/jprodger/Bumblebee/src/new_features/tune_output/'
SAVE_DIR = r'/depot/cms/top/jprodger/Bumblebee/src/new_features/plot_output/'
#############################################################
...
if get_data_from_file:
    ...
    truths = torch.load(BASE_DIR + 'truths0.pt')
    predictions = torch.load(BASE_DIR + 'predictions0.pt')
    
    tt_info = torch.load(BASE_DIR + 'reco_tops0.pt')
    ...
    #for i in tqdm(range(1000)):
        ...
        for j in range(B):
            ...
            gen_l_pt.append((truths[i][j, -7, 0] * gen_stdevs[0, 0]) + gen_means[0, 0])
            gen_lbar_pt.append((truths[i][j, -6, 0] * gen_stdevs[1, 0]) + gen_means[1, 0])
            gen_b_pt.append((truths[i][j, -5, 0] * gen_stdevs[2, 0]) + gen_means[2, 0])
            gen_bbar_pt.append((truths[i][j, -4, 0] * gen_stdevs[3, 0]) + gen_means[3, 0])
            gen_n_pt.append((truths[i][j, -3, 0] * gen_stdevs[4, 0]) + gen_means[4, 0])
            gen_nbar_pt.append((truths[i][j, -2, 0] * gen_stdevs[5, 0]) + gen_means[5, 0])

            gen_l_eta.append((truths[i][j, -7, 1] * gen_stdevs[0, 1]) + gen_means[0, 1])
            gen_lbar_eta.append((truths[i][j, -6, 1] * gen_stdevs[1, 1]) + gen_means[1, 1])
            gen_b_eta.append((truths[i][j, -5, 1] * gen_stdevs[2, 1]) + gen_means[2, 1])
            gen_bbar_eta.append((truths[i][j, -4, 1] * gen_stdevs[3, 1]) + gen_means[3, 1])
            gen_n_eta.append((truths[i][j, -3, 1] * gen_stdevs[4, 1]) + gen_means[4, 1])
            gen_nbar_eta.append((truths[i][j, -2, 1] * gen_stdevs[5, 1]) + gen_means[5, 1])

            gen_l_phi.append((truths[i][j, -7, 2] * gen_stdevs[0, 2]) + gen_means[0, 2])
            gen_lbar_phi.append((truths[i][j, -6, 2] * gen_stdevs[1, 2]) + gen_means[1, 2])
            gen_b_phi.append((truths[i][j, -5, 2] * gen_stdevs[2, 2]) + gen_means[2, 2])
            gen_bbar_phi.append((truths[i][j, -4, 2] * gen_stdevs[3, 2]) + gen_means[3, 2])
            gen_n_phi.append((truths[i][j, -3, 2] * gen_stdevs[4, 2]) + gen_means[4, 2])
            gen_nbar_phi.append((truths[i][j, -2, 2] * gen_stdevs[5, 2]) + gen_means[5, 2])

            gen_l_mass.append((truths[i][j, -7, 3] * gen_stdevs[0, 3]) + gen_means[0, 3])
            gen_lbar_mass.append((truths[i][j, -6, 3] * gen_stdevs[1, 3]) + gen_means[1, 3])
            gen_b_mass.append((truths[i][j, -5, 3] * gen_stdevs[2, 3]) + gen_means[2, 3])
            gen_bbar_mass.append((truths[i][j, -4, 3] * gen_stdevs[3, 3]) + gen_means[3, 3])
            gen_n_mass.append((truths[i][j, -3, 3] * gen_stdevs[4, 3]) + gen_means[4, 3])
            gen_nbar_mass.append((truths[i][j, -2, 3] * gen_stdevs[5, 3]) + gen_means[5, 3])


            # RECO COMPONENTS
            
            reco_l_pt.append((truths[i][j, 1, 0] * reco_stdevs[0, 0]) + reco_means[0, 0])
            reco_lbar_pt.append((truths[i][j, 2, 0] * gen_stdevs[1, 0]) + reco_means[1, 0])
            reco_b_pt.append((truths[i][j, 3, 0] * gen_stdevs[2, 0]) + reco_means[2, 0])
            reco_bbar_pt.append((truths[i][j, 4, 0] * reco_stdevs[3, 0]) + reco_means[3, 0])
            reco_met_pt.append((truths[i][j, -9, 0] * reco_stdevs[4, 0]) + reco_means[4, 0])
            
            temp_reco_l_eta = (truths[i][j, 1, 1] * reco_stdevs[0, 1]) + reco_means[0, 1]
            temp_reco_lbar_eta = (truths[i][j, 2, 1] * gen_stdevs[1, 1]) + reco_means[1, 1]

            reco_l_eta.append(temp_reco_l_eta)
            reco_lbar_eta.append(temp_reco_lbar_eta)
            reco_b_eta.append((truths[i][j, 3, 1] * gen_stdevs[2, 1]) + reco_means[2, 1])
            reco_bbar_eta.append((truths[i][j, 4, 1] * reco_stdevs[3, 1]) + reco_means[3, 1])
            reco_met_eta.append((truths[i][j, -9, 1] * reco_stdevs[4, 1]) + reco_means[4, 1])
            
            temp_reco_l_phi = (truths[i][j, 1, 2] * reco_stdevs[0, 2]) + reco_means[0, 2]
            temp_reco_lbar_phi = (truths[i][j, 2, 2] * gen_stdevs[1, 2]) + reco_means[1, 2]
            
            reco_l_phi.append(temp_reco_l_phi)
            reco_lbar_phi.append(temp_reco_lbar_phi)
            reco_b_phi.append((truths[i][j, 3, 2] * gen_stdevs[2, 2]) + reco_means[2, 2])
            reco_bbar_phi.append((truths[i][j, 4, 2] * reco_stdevs[3, 2]) + reco_means[3, 2])
            reco_met_phi.append((truths[i][j, -9, 2] * reco_stdevs[4, 2]) + reco_means[4, 2])

            reco_l_mass.append((truths[i][j, 1, 3] * reco_stdevs[0, 3]) + reco_means[0, 3])
            reco_lbar_mass.append((truths[i][j, 2, 3] * gen_stdevs[1, 3]) + reco_means[1, 3])
            reco_b_mass.append((truths[i][j, 3, 3] * gen_stdevs[2, 3]) + reco_means[2, 3])
            reco_bbar_mass.append((truths[i][j, 4, 3] * reco_stdevs[3, 3]) + reco_means[3, 3])
            reco_met_mass.append((truths[i][j, -9, 3] * reco_stdevs[4, 3]) + reco_means[4, 3])
            
            
            # PRED COMPONENTS
            
            pred_l_pt.append((predictions[i][j, -7, 0] * gen_stdevs[0, 0]) + gen_means[0, 0])
            pred_lbar_pt.append((predictions[i][j, -6, 0] * gen_stdevs[1, 0]) + gen_means[1, 0])
            pred_b_pt.append((predictions[i][j, -5, 0] * gen_stdevs[2, 0]) + gen_means[2, 0])
            pred_bbar_pt.append((predictions[i][j, -4, 0] * gen_stdevs[3, 0]) + gen_means[3, 0])
            pred_n_pt.append((predictions[i][j, -3, 0] * gen_stdevs[4, 0]) + gen_means[4, 0])
            pred_nbar_pt.append((predictions[i][j, -2, 0] * gen_stdevs[5, 0]) + gen_means[5, 0])

            pred_l_eta.append(temp_reco_l_eta)
            pred_lbar_eta.append(temp_reco_lbar_eta)
            pred_b_eta.append((predictions[i][j, -5, 1] * gen_stdevs[2, 1]) + gen_means[2, 1])
            pred_bbar_eta.append((predictions[i][j, -4, 1] * gen_stdevs[3, 1]) + gen_means[3, 1])
            pred_n_eta.append((predictions[i][j, -3, 1] * gen_stdevs[4, 1]) + gen_means[4, 1])
            pred_nbar_eta.append((predictions[i][j, -2, 1] * gen_stdevs[5, 1]) + gen_means[5, 1])

            pred_l_phi.append(temp_reco_l_phi)
            pred_lbar_phi.append(temp_reco_lbar_phi)
            pred_b_phi.append((predictions[i][j, -5, 2] * gen_stdevs[2, 2]) + gen_means[2, 2])
            pred_bbar_phi.append((predictions[i][j, -4, 2] * gen_stdevs[3, 2]) + gen_means[3, 2])
            pred_n_phi.append((predictions[i][j, -3, 2] * gen_stdevs[4, 2]) + gen_means[4, 2])
            pred_nbar_phi.append((predictions[i][j, -2, 2] * gen_stdevs[5, 2]) + gen_means[5, 2])
            
            temp_pred_l_mass = (predictions[i][j, -7, 3] * gen_stdevs[0, 3]) + gen_means[0, 3]
            temp_pred_lbar_mass = (predictions[i][j, -6, 3] * gen_stdevs[1, 3]) + gen_means[1, 3]
            
            if temp_pred_l_mass > muon_mass:
                temp_pred_l_mass = muon_mass
            elif temp_pred_l_mass < e_mass:
                temp_pred_l_mass = e_mass
            elif temp_pred_lbar_mass > muon_mass:
            ...
            pred_b_mass.append((predictions[i][j, -5, 3] * gen_stdevs[2, 3]) + gen_means[2, 3])
            pred_bbar_mass.append((predictions[i][j, -4, 3] * gen_stdevs[3, 3]) + gen_means[3, 3])
            pred_n_mass.append((predictions[i][j, -3, 3] * gen_stdevs[4, 3]) + gen_means[4, 3])
            pred_nbar_mass.append((predictions[i][j, -2, 3] * gen_stdevs[5, 3]) + gen_means[5, 3])
            ...
            t_temp_reco = vector.obj(pt=tt_info[i][j, 0, 0], phi = tt_info[i][j, 0, 2], eta = tt_info[i][j, 0, 1], mass = tt_info[i][j, 0, 3])
            ...
            tbar_temp_reco = vector.obj(pt=tt_info[i][j, 1, 0], phi = tt_info[i][j, 1, 2], eta = tt_info[i][j, 1, 1], mass = tt_info[i][j, 1, 3])

            # ttbar combined
            ttbar_temp_pred = t_temp_pred + tbar_temp_pred
            ttbar_temp_gen = t_temp_gen + tbar_temp_gen
            ttbar_temp_reco = t_temp_reco + tbar_temp_reco
            ...
            reco_t_mass.append(t_temp_reco.mass)
            reco_t_pt.append(t_temp_reco.pt)
            reco_t_eta.append(t_temp_reco.eta)
            reco_t_phi.append(t_temp_reco.phi)
            reco_t_px.append(t_temp_reco.px)
            reco_t_py.append(t_temp_reco.py)
            reco_t_pz.append(t_temp_reco.pz)
    
            reco_tbar_mass.append(tbar_temp_reco.mass)
            reco_tbar_pt.append(tbar_temp_reco.pt)
            reco_tbar_eta.append(tbar_temp_reco.eta)
            reco_tbar_phi.append(tbar_temp_reco.phi)
            reco_tbar_px.append(tbar_temp_reco.px)
            reco_tbar_py.append(tbar_temp_reco.py)
            reco_tbar_pz.append(tbar_temp_reco.pz)
            ...
            reco_ttbar_mass.append(ttbar_temp_reco.mass)
            reco_ttbar_pt.append(ttbar_temp_reco.pt)
            reco_ttbar_eta.append(ttbar_temp_reco.eta)
            reco_ttbar_phi.append(ttbar_temp_reco.phi)
            reco_ttbar_px.append(ttbar_temp_reco.px)
            reco_ttbar_py.append(ttbar_temp_reco.py)
            reco_ttbar_pz.append(ttbar_temp_reco.pz)
...
reconstructed_events_file["Bumblebee"] = {
            ...
            "reco_t_pt": reco_t_pt,
            "reco_t_eta": reco_t_eta,
            "reco_t_phi": reco_t_phi,
            "reco_t_mass": reco_t_mass,
            "reco_t_px": reco_t_px,
            "reco_t_py": reco_t_py,
            "reco_t_pz": reco_t_pz,
            "reco_tbar_pt": reco_tbar_pt,
            "reco_tbar_eta": reco_tbar_eta,
            "reco_tbar_phi": reco_tbar_phi,
            "reco_tbar_mass": reco_tbar_mass,
            "reco_tbar_px": reco_tbar_px,
            "reco_tbar_py": reco_tbar_py,
            "reco_tbar_pz": reco_tbar_pz,
            "reco_ttbar_pt": reco_ttbar_pt,
            "reco_ttbar_eta": reco_ttbar_eta,
            "reco_ttbar_phi": reco_ttbar_phi,
            "reco_ttbar_mass": reco_ttbar_mass,
            "reco_ttbar_px": reco_ttbar_px,
            "reco_ttbar_py": reco_ttbar_py,
            "reco_ttbar_pz": reco_ttbar_pz,
...
if post_as_pngs:
    ...
    fig64 = plt.figure()
    a = plt.hist2d(gen_ttbar_mass, pred_ttbar_mass, bins=100, range=[[200, 600],[200, 600]], 
                  norm=mpl.colors.LogNorm())
    plt.colorbar(a[3])
    ...
    fig68 = plt.figure()
    a = plt.hist2d(reco_ttbar_mass, pred_ttbar_mass, bins=100, range=[[200, 600],[200, 600]], 
                  norm=mpl.colors.LogNorm())
    plt.colorbar(a[3])
    plt.title('2D ttbar mass plot')
    plt.xlabel(r'reco ttbar mass [GeV]')
    plt.ylabel(r'pred ttbar mass [GeV]')
    
    fig69 = plt.figure()
    a = plt.hist2d(reco_ttbar_phi, pred_ttbar_phi, bins=100, range=[[-3.15, 3.15],[-3.15, 3.15]], 
                  norm=mpl.colors.LogNorm())
    plt.colorbar(a[3])
    plt.title('2D ttbar phi plot')
    plt.xlabel(r'reco ttbar phi')
    plt.ylabel(r'pred ttbar phi')
    
    fig70 = plt.figure()
    a = plt.hist2d(reco_ttbar_eta, pred_ttbar_eta, bins=100, range=[[-10, 10],[-10, 10]], 
                  norm=mpl.colors.LogNorm())
    plt.colorbar(a[3])
    plt.title('2D ttbar eta plot')
    plt.xlabel(r'reco ttbar eta')
    plt.ylabel(r'pred ttbar eta')
    
    fig71 = plt.figure()
    a = plt.hist2d(reco_ttbar_pt, pred_ttbar_pt, bins=100, range=[[0, 1000],[0, 1000]], 
                  norm=mpl.colors.LogNorm())
    plt.colorbar(a[3])
    plt.title('2D ttbar pT plot')
    plt.xlabel(r'reco ttbar pT [GeV]')
    plt.ylabel(r'pred ttbar pT [GeV]')
    
    
    # make pdf
    
    p = PdfPages(SAVE_DIR + 'reco_plots.pdf')
