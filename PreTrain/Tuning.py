...
parser.set_defaults(d_model=512, dropout = 0.1,
                    ...
                    N = 12, h = 16, warmup=30000,
                    data_dir = r'/depot/cms/top/jprodger/Bumblebee/src/reco_data2/',
                    save_dir = r'/depot/cms/top/jprodger/Bumblebee/src/new_features/tune_output/',
...
def train(trial):
    # writing config file for loading later
    file = open(args.save_dir + f"bumblebee{trial.number}_config.txt", "w")
    m_params = [
        str(args.batch_size) + "\n",
        str(args.d_model) + "\n",
        str(args.dropout) + "\n",
    ...
    file.writelines(m_params)
    file.close()
    
    args.d_model = trial.suggest_categorical("d_model", [128, 256, 512, 1024])
    args.dropout = trial.suggest_float("dropout", 0.0, 0.8)
    args.batch_size = trial.suggest_categorical("batch_size", [64, 128, 256])
    args.N = trial.suggest_int("N", 3, 12)
    args.h = trial.suggest_categorical("h", [2, 4, 8, 16, 32, 64])
    args.weight_decay = trial.suggest_float("weight_decay", 1e-5, 1e-1, log=True)
    args.epsilon = trial.suggest_float("epsilon", 1e-9, 1e-4, log=True)
    args.warmup = trial.suggest_int("warmup", 1000, 100000)
    ...
    wandb.init(
        project='BUMBLEBEE_TUNING W_NEW_FEATURES_2',
    ...
    for t in range(args.n_epochs):
        ...
        for data_loader in train_data_loaders:
            for batch_id, (x, target, _) in tqdm(enumerate(data_loader), desc="TRAINING"):
                ...
                '''
                # llbar eta, phi
                four_vector_mask[:, -7:-5, 1] = torch.ones(four_vector_mask[:, -7:-5, 1].shape)
                four_vector_mask[:, -7:-5, 2] = torch.ones(four_vector_mask[:, -7:-5, 2].shape)
                '''
        ...
        with torch.no_grad():
            ...
            for valid_data_loader in valid_data_loaders:
                for valid_batch_id, (x_valid, target_valid, _) in tqdm(enumerate(valid_data_loader), desc="VALIDATING"):
                    ...
                    # MET / Neutrino masking
                    four_vector_mask_valid[:, -9, 1] = torch.ones(four_vector_mask_valid[:, -9, 1].shape)
                    four_vector_mask_valid[:, -9, 3] = torch.ones(four_vector_mask_valid[:, -9, 3].shape)
                    four_vector_mask_valid[:, -3:-1, 1] = torch.ones(four_vector_mask_valid[:, -3:-1, 1].shape)
                    four_vector_mask_valid[:, -3:-1, 3] = torch.ones(four_vector_mask_valid[:, -3:-1, 3].shape)
                    '''
                    # llbar eta, phi
                    four_vector_mask_valid[:, -7:-5, 1] = torch.ones(four_vector_mask_valid[:, -7:-5, 1].shape)
                    four_vector_mask_valid[:, -7:-5, 2] = torch.ones(four_vector_mask_valid[:, -7:-5, 2].shape)
                    '''
    ...
    if torch.cuda.device_count() > 1:
        best_model.module.load_state_dict(torch.load(args.save_dir + f'bumblebee{trial.number}.pt'))
    else:
        best_model.load_state_dict(torch.load(args.save_dir + f'bumblebee{trial.number}.pt'))
    ...
    for test_data_loader in test_data_loaders:
        for test_batch_id, (x_test, target_test, reco_top_test) in tqdm(enumerate(test_data_loader), desc="TESTING"):
            ...
            # MET / Neutrino masking
            four_vector_mask_test[:, -9, 1] = torch.ones(four_vector_mask_test[:, -9, 1].shape)
            four_vector_mask_test[:, -9, 3] = torch.ones(four_vector_mask_test[:, -9, 3].shape)
            four_vector_mask_test[:, -3:-1, 1] = torch.ones(four_vector_mask_test[:, -3:-1, 1].shape)
            four_vector_mask_test[:, -3:-1, 3] = torch.ones(four_vector_mask_test[:, -3:-1, 3].shape)
            '''
            # llbar eta, phi
            four_vector_mask_test[:, -7:-5, 1] = torch.ones(four_vector_mask_test[:, -7:-5, 1].shape)
            four_vector_mask_test[:, -7:-5, 2] = torch.ones(four_vector_mask_test[:, -7:-5, 2].shape)
            '''
    ...
    # Training
    torch.save(np.array(masked_training_losses), args.save_dir + f'used_training_losses{trial.number}.pt')
    torch.save(np.array(training_losses), args.save_dir + f'net_training_losses{trial.number}.pt')
    torch.save(np.array(training_loss_components), args.save_dir + f'training_loss_components{trial.number}.pt')
    
    # Validation
    torch.save(np.array(validation_losses), args.save_dir + f'validation_losses{trial.number}.pt')
    torch.save(np.array(masked_validation_losses), args.save_dir + f'masked_validation_losses{trial.number}.pt')
    
    # Test
    torch.save(np.array(test_losses), args.save_dir + f'test_losses{trial.number}.pt')
    torch.save(np.array(masked_test_losses), args.save_dir + f'masked_test_losses{trial.number}.pt')
    torch.save(np.array(test_loss_components), args.save_dir + f'test_loss_components{trial.number}.pt')
    ...
    torch.save(np.array(test_reco_tops), args.save_dir + f'reco_tops{trial.number}.pt')
    
    wandb.finish()
    return avg_masked_test_loss
    ### AND WE'RE DONE!!!

study = optuna.create_study(study_name = "BB TUNING", direction = "minimize", load_if_exists = True, storage = "sqlite:///bb_tuning.db")
