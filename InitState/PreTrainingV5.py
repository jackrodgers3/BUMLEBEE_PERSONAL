...
parser.set_defaults(d_model=512, dropout = 0.1,
                    batch_size = 64, n_epochs = 2, only_gen = False,
                    ...
                    save_dir = r'/depot/cms/top/jprodger/Bumblebee/src/initial_state_discrimination2/output/',
...
wandb.init(
    project='BUMBLEBEE_INIT_STATE_W_NEW_FEATURES',
...
if torch.cuda.device_count() > 1:
    disc_model.module.load_state_dict(torch.load(args.save_dir + 'bumblebee.pt'))
else:
    disc_model.load_state_dict(torch.load(args.save_dir + 'bumblebee.pt'))

d_train_events, d_valid_events, d_test_events = train_valid_test(test_events, args.tvt_split, use_generator = False)
...
for t in range(args.n_epochs):
    ...
    if avg_valid_loss < best_valid_loss or best_valid_loss == -1:
        best_validation_loss = avg_masked_valid_loss
        if torch.cuda.device_count() > 1:
            torch.save(disc_model.module.state_dict(), args.save_dir + 'bumblebee_disc.pt')
        else:
            torch.save(disc_model.state_dict(), args.save_dir + 'bumblebee_disc.pt')
...
if torch.cuda.device_count() > 1:
    disc_best_model.module.load_state_dict(torch.load(args.save_dir + 'bumblebee_disc.pt'))
else:
    disc_best_model.load_state_dict(torch.load(args.save_dir + 'bumblebee_disc.pt'))
...
with torch.no_grad():
    for test_data_loader in test_data_loaders:
        for test_batch_id, (x_test, target_test) in tqdm(enumerate(test_data_loader), desc = "TESTING"):
            ...
            disc_truths.append(torch.sigmoid(corrected_pred_test[:, 0, 0]).detach().cpu().numpy())
            disc_predictions.append(target_test.detach().cpu().numpy())
