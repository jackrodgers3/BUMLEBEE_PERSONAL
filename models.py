...
def attention(query, key, value, h = 8, mask=None, dropout=None):
    ...
    scores = h * torch.matmul(query, key.transpose(-2, -1)) \
             / d_k
    if mask is not None:
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, bias=True):
        ...
        self.init_method = init_method_normal((1.0 / d_model)**0.5)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.attn = attention(query, key, value, h=self.h, mask=mask,
...
class ParticleEventTransformer(nn.Module):
    def __init__(self, particle_dimensionality, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1, bias=True,
                 mask_particle_embedding=True,
                 mask_all_but_pid_embedding=False):
        super(ParticleEventTransformer, self).__init__()
        c = copy.deepcopy
        self.d_model = d_model
        self.bias = bias
        attn = MultiHeadedAttention(h, d_model, dropout, bias)
        ff = PositionwiseFeedForward(d_model, d_ff, dropout, bias)
        ...
        self.particle_embedding_init_method = init_method_normal((1.0 / particle_dimensionality)**0.5)
    ...
    def forward(self, x):
        ...
        return self.reco_layer(encoding)
...
def make_model(particle_dimensionality=4, N=6, 
               d_model=512, d_ff=2048, h=8, dropout=0.1, mask_particle_embedding=True,
               mask_all_but_pid_embedding=False):
    "Helper: Construct a model from hyperparameters."
    c = copy.deepcopy
    model = ParticleEventTransformer(particle_dimensionality, N=N, d_model=d_model, d_ff=d_ff, h=h,
        dropout=dropout, mask_particle_embedding=True, mask_all_but_pid_embedding=False)
