...
parser.set_defaults(d_model=512, dropout = 0.1,
                    ...
                    save_dir = r'/depot/cms/top/jprodger/Bumblebee/src/disc_study/output/',
...
wandb.init(
    project='BUMBLEBEE_DISCRIMINATION',
...
for t in range(args.n_epochs):
    ...
    with torch.no_grad():
        ...
        # plt.show()
        plt.savefig(args.save_dir + f'hist_plots@{t}.pdf')
        plt.clf()
        plt.cla()
...
model = make_model(particle_dimensionality=4, N=args.N, d_model=args.d_model,
                   d_ff= int(4 * args.d_model), h = args.h, dropout= args.dropout, act_fn = args.act_fn)
if torch.cuda.device_count() > 1:
    model.module.load_state_dict(torch.load(args.save_dir + 'bumblebee.pt'))
else:
    model.load_state_dict(torch.load(args.save_dir + 'bumblebee.pt'))
if torch.cuda.device_count() > 1:
    print(f"USING {torch.cuda.device_count()} GPUS")
    model = nn.DataParallel(model)
model = model.to(device)
...
for t in range(5):
    ...
    with torch.no_grad():
        ...
        if avg_valid_loss < best_validation_loss or best_validation_loss == -1:
            ...
            if torch.cuda.device_count() > 1:
                torch.save(model.module.state_dict(), args.save_dir + 'bumblebee_cls.pt')
            else:
                torch.save(model.state_dict(), args.save_dir + 'bumblebee_cls.pt')
...
if torch.cuda.device_count() > 1:
    best_model.module.load_state_dict(torch.load(args.save_dir + 'bumblebee_cls.pt'))
else:
    best_model.load_state_dict(torch.load(args.save_dir + 'bumblebee_cls.pt'))
