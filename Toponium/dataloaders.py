...
class GenRecoDataset(Dataset):
    ...
    def __init__(self, events, gen_reco_ids, standardize=True, pretraining=True, ids=None, mask_probability=0.15):
        ...
        if standardize:
            ...
            self.gen_means = torch.mean(self.gen_four_vectors, axis=0)
            self.gen_stdevs = torch.std(self.gen_four_vectors, 0, True)
            # equating llbar phi gen stats to reco stats
            self.gen_means[1:3, 2] = self.reco_means[1:3, 2]
            self.gen_stdevs[1:3, 2] = self.reco_stdevs[1:3, 2]
            # equating llbar eta gen stats to reco stats
            self.gen_means[1:3, 1] = self.reco_means[1:3, 1]
            self.gen_stdevs[1:3, 1] = self.reco_stdevs[1:3, 1]
        ...
        # instantiating just to be safe
        self.t_info = None
        self.gen_four_vectors = None
    ...
    def __getitem__(self, index):
        ...
        four_vector_mask = zerod_mask[:, None].repeat(1, 4)
        four_vector_mask[11:13, 3] = torch.ones(four_vector_mask[11:13, 3].shape) # never mask neutrino mass
        four_vector_mask[7:9, 1:3] = torch.ones(four_vector_mask[7:9, 1:3].shape) # never mask llbar eta, phi
        masked_four_vectors = (self.four_vectors[index] * four_vector_mask)

        return torch.cat((self.ids[index][:, None], self.gen_reco_ids[:, None], masked_four_vectors, zerod_mask[:, None]), axis=1), self.four_vectors[index]
        
    def get_reco_top_info(self):
        self.t_info = torch.cat(
            (
                torch.cat(
                    (
                        torch.from_numpy(self.events['top_pt'])[:, None],
                        torch.from_numpy(self.events['top_eta'])[:, None],
                        torch.from_numpy(self.events['top_phi'])[:, None],
                        torch.from_numpy(self.events['top_mass'])[:, None]
                    ),
                    axis=1
                )[:, :, None],
                torch.cat(
                    (
                        torch.from_numpy(self.events['tbar_pt'])[:, None],
                        torch.from_numpy(self.events['tbar_eta'])[:, None],
                        torch.from_numpy(self.events['tbar_phi'])[:, None],
                        torch.from_numpy(self.events['tbar_mass'])[:, None]
                    ),
                    axis=1
                )[:, :, None],
            ),
...
class GenRecoDataset_Toponium(Dataset):
    """
    The Generator-Reconstruction level dataset used for training particle physics event encodings.
    Args:
        :param events: The uproot file containing all the events to be trained on
    ...
    def __init__(self, events, gen_reco_ids, standardize=True, pretraining=True, ids=None, mask_probability=0.15):
        ...
        self.targets = torch.from_numpy(self.events['bs'])
    ...
    def __getitem__(self, index):
        ...
        return torch.cat((self.ids[index][:, None], self.gen_reco_ids[:, None], masked_four_vectors, zerod_mask[:, None]), axis=1), self.targets[index]


class GenRecoDataset_InitState(Dataset):
    """
    The Generator-Reconstruction level dataset used for training particle physics event encodings.
    Args:
        :param events: The uproot file containing all the events to be trained on
    ...
    def __init__(self, events, gen_reco_ids, standardize=True, pretraining=True, ids=None, mask_probability=0.15):
        ...
        self.targets = torch.from_numpy(self.events['TopProductionMode'])
