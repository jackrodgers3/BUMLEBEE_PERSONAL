...
def create_data(events, standardize, pretraining, mask_probability, batch_size, tag):
    ...
    for i in tqdm(range(B), desc="EDITING FOR JETS"):
        ...
        # getting top ids
        tops.append(events['toponium_flag'][i])
        # editing ids
    ...
    for i in tqdm(range(B), desc="CONCAT AND MASKING"):
        ...
        unique_sizes[D-5].append([ids_w_jets[i], gen_reco_ids_w_jets[i], recos_w_jets[i], gen_four_vectors[i], tops[i]])
    t_dataloaders = []
...
parser.set_defaults(d_model=256, dropout = 0.1,
                    ...
                    data_dir = r'/depot/cms/top/colberte/DelphesProduction/Processing/delphes_toponium_sample/',
                    save_dir = r'/depot/cms/top/jprodger/Bumblebee/src/Paper_Delphes_Stuff/finetune_top_disc/output/',
...
background_events = torch.load(args.load_dir + 'test_data.pt')

notau_filenames1 = {
    f'{args.data_dir}/delphes_*': f'Step7;1'
}
...
background_events['toponium_flag'] = [0. for _ in range(len(background_events['l_pt']))]
signal_events['toponium_flag'] = [1. for _ in range(len(signal_events['l_pt']))]

total_events = merge_events([background_events, signal_events])
...
wandb.init(
    project='Bumblebee_DP_top_disc_finetuning',
