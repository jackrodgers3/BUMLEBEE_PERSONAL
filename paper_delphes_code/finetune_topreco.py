...
parser.add_argument('--load_dir', help='directory to load pretrained model from', type=str)
...
parser.set_defaults(d_model=256, dropout = 0.1,
                    ...
                    save_dir = r'/depot/cms/top/jprodger/Bumblebee/src/Paper_Delphes_Stuff/top_reco/output/',
...
print("DATA PRESENT")
print("Counts: ", len(total_events['l_pt']))

test_data_loaders = create_data(events=total_events, standardize=args.standardize,
...
wandb.init(
    project='Bumblebee_DP_finetune_top_reco',
...
if torch.cuda.device_count() > 1:
    best_model.module.load_state_dict(torch.load(args.load_dir + 'bumblebee.pt'))
else:
    best_model.load_state_dict(torch.load(args.load_dir + 'bumblebee.pt'))
